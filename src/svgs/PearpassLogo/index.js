import { html } from 'htm/react'

/**
 * Hatch plate + Lockwright wordmark. Export name stays PearpassLogo
 * because callers and the ui-kit icon still use that identifier.
 */
export const PearpassLogo = ({ width = '160', height = '32' }) => html`
  <svg
    width=${width}
    height=${height}
    viewBox="0 0 220 64"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    role="img"
    aria-label="Lockwright"
  >
    <rect
      x="4"
      y="4"
      width="56"
      height="56"
      rx="2"
      fill="#14161b"
      stroke="#b08d57"
      stroke-width="1.5"
    />
    <rect
      x="14"
      y="14"
      width="36"
      height="36"
      rx="2"
      fill="#08090b"
      stroke="#2a2e36"
      stroke-width="1"
    />
    <rect x="25" y="20" width="14" height="14" rx="2" fill="#d4af77" />
    <rect x="29" y="32" width="6" height="14" rx="1" fill="#d4af77" />
    <text
      x="72"
      y="42"
      fill="#F6F6F6"
      font-family="Inter, system-ui, sans-serif"
      font-size="26"
      font-weight="500"
    >
      Lockwright
    </text>
  </svg>
`

export const LockwrightMark = ({
  width = '24',
  height = '24',
  color = '#b08d57'
}) => html`
  <svg
    width=${width}
    height=${height}
    viewBox="0 0 64 64"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    role="img"
    aria-label="Lockwright"
  >
    <rect
      x="4"
      y="4"
      width="56"
      height="56"
      rx="2"
      fill="#14161b"
      stroke=${color}
      stroke-width="1.5"
    />
    <rect
      x="14"
      y="14"
      width="36"
      height="36"
      rx="2"
      fill="#08090b"
      stroke="#2a2e36"
      stroke-width="1"
    />
    <rect x="25" y="20" width="14" height="14" rx="2" fill=${color} />
    <rect x="29" y="32" width="6" height="14" rx="1" fill=${color} />
  </svg>
`
